...
@Component({
  selector: 'cn-dynamic-template-layout,[cn-dynamic-template-layout]',
  templateUrl: './dynamic-template-layout.component.html',
  styles: [``],
})
export class CnDynamicTemplateLayoutComponent extends CnComponentBase implements OnInit {
  @Input()
  ...
  private loadDynamicLayout(layoutId: string) {
    if (environment.systemSettings && environment.systemSettings.systemMode === 'work') {
      ...
      let page_params = { PageId: layoutId };
