...
@Component({
  selector: 'app-smt-data-table-month-picker',
  templateUrl: './smt-data-table-month-picker.component.html',
  styles: [
  ]
})
export class SmtDataTableMonthPickerComponent implements OnInit {
  @Input() config;
  ...
  ngOnInit(): void {
    if (!this.config.showFormat) {
      this.config.showFormat = 'yyyy-MM';
    }
    let v_value;
    if (this.valueConfig) {
