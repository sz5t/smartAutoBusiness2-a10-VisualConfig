...
@Component({
  selector: 'app-cn-cfg-flow-preview',
  templateUrl: './cn-cfg-flow-preview.component.html',
  styles: [
  ]
})
export class CnCfgFlowPreviewComponent extends CnComponentBase implements OnInit, OnDestroy, AfterViewInit {
  ...
  style_sider = { height: (window.document.body.clientHeight - 160).toString() + 'px' }

  drawerHeight = window.document.body.clientHeight - 200;
  ngOnInit(): void {
    if (this.initData) {
  ...
  public async ngAfterViewInit() {
    ...
    let _width = (window.document.body.clientWidth - 100);
    ...
    this.graph.centerContent({ padding: { left: 100 } });
  ...
  public async load() {
    ...
    if (c_flows) {
      this.update_edges_state_array(c_flows);
    }
  ...
  public async load_detail(key?) {
    this.sourceData = null;
    let loadingDetailConfig = {
      "enableAjaxMore": true,
      "url": "smt-jbpm/process/track/detail/query/{procinstId}/{taskKey}",
      "ajaxType": "get",
      "pathParams": [
        {
          "name": "procinstId",
          "type": "tempValue",
          "valueName": "PROCINST_ID",
          "value": -1
        },
        {
          "name": "taskKey",
    ...
    let data_form;
    let c_flows;
    if (loadingDetailConfig) {

      if (loadingDetailConfig['enableAjaxMore']) {
        response = await this.executeHttpMore(loadingDetailConfig, {}, 'buildParameters', null);
      }
    ...
    this.form_process_config = JSON.parse(JSON.stringify(this.form_process_config))
    console.log('加载节点明细===》》》', data_form, this.sourceData)
  ...
  public update_edges_state_array(strs?) {
    ...
    "properties": [
      {
        ...
        "componentConfig": {
          "active": true,
          "defaultActionAddHidden": true,
          "defaultActionDeleteHidden": true,
          "mode": "empty",
