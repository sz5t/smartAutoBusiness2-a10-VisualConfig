...
export class CnTimeAxisChartComponent extends CnComponentBase implements OnInit, AfterViewInit {
  @Input() public config;
  @Input() tempData;
  @Input() initData;
  ...
  public initComponentValue() {
    ...
    if (this.config.BasiAttribute.y1) {
      this.Y1 = this.config.BasiAttribute.y1.name

      this.Y1scale = this.config.BasiAttribute.y1['scale'] ? this.config.BasiAttribute.y1['scale'] : null

      this.Y1axis = this.config.BasiAttribute.y1['axis'] ? this.config.BasiAttribute.y1['axis'] : null
    ...
    if (this.config.BasiAttribute.groupName) {
      this.groupName = this.config.BasiAttribute.groupName
    }
  }
 ...
 */
  ...
  public InitAxis() {
    ...
    if (this.Y1) {
      if (this.Y1axis) {
        this.chart.axis(this.Y1, this.Y1axis);
      }
      if (this.Y1scale) {
        this.chart.scale(this.Y1, this.Y1scale);
      }
    }
  }
 ...
 */
  public createSlider(data?) {
    ...
    //     [this.X]: {
    //       formatter: (val) => {
    //         return `${getYear(val)}-${getMonth(val) + 1}-${getDate(val)}${' '}${getHours(getTime(val))}${':'}${getMinutes(getTime(val))}${':'}${getSeconds(getTime(val))}`;
    //       }
    //     }
    //   },
    //   onChange: (_ref) => {
  ...
  public generateCharts() {
    const guideCfgId = this.config.BasiAttribute.overid ? this.config.BasiAttribute.overid : ''
    let guideCfg
    if (this.config.showUserPoint) {
      if (this.config.BasiAttribute.groupName) {
        ...
        if (this.config.overConfig) {
          guideCfg = this.config.overConfig.find(e => e.id === guideCfgId);
  ...
  public registerPointStyle() {
    ...
    G2.registerShape('point', 'overY', {
      draw(cfg, container) {
        ...
        if (condition === 1) {
          if (guideCfgId !== '') {
            guideCfg = that.config.overConfig.find(e => e.id === guideCfgId)
          }
          color = guideCfg.y
    ...
    // 不符合计划表中对应点的数据
    G2.registerShape('point', 'overDataRule', {
      draw(cfg, container) {
        const data = cfg.data;
        const point = { x: cfg.x, y: cfg.y };
        let condition = 0; // 判断规则值数组和数据源的条件关系
