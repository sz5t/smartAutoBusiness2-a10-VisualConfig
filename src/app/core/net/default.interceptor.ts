...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (!url.startsWith('https://') && !url.startsWith('http://')) {
      if (url.startsWith('/mock')) {
        ...
        url = this._mappingResourceServer(url);
      }
    }
  ...
  private _mappingResourceServer(url: string): string {
    if (environment.systemSettings.enableResourceProx) {
      const mData = this.cacheService.getNone('resourceMappingData');
      const urlArr = url.split('/');
      const newHead = mData[urlArr[2]];
      let newUrl: string = '';
      if (newHead) {
